...
export default function AddNodeFloatingMenu({ onNodeAdd }: AddNodeFloatingMenuProps) {
    const [showMenu, setShowMenu, anchorPosition, _incomingNodeMetadetails] = useAddNodeOnEdgeDropState(s => [s.showMenu, s.actions.setShowMenu, s.anchorPosition, s.incomingNodeMetadetails]);
    ...
    const nodesWithMetadata = useMemo(
        () => {
            const mapNodesWithMetadata = (types: BuilderNodeType[]): Array<typeof AVAILABLE_NODES[number] & { __meta: typeof NODES_METADATA[BuilderNodeType]; __enabled: boolean }> => types.map((type) => {
